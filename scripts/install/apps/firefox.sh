...
fingerprint_match=$(gpg -n -q --import --import-options import-show /etc/apt/keyrings/packages.mozilla.org.asc \
  | awk '/pub/{getline; gsub(/^ +| +$/,""); if($0 == "35BAA0B33E9EB396F59CA838C0BA5CE6DC6315A3") print "good"; else print "bad"}')
...
' | tee /etc/apt/preferences.d/mozilla
  ...
  echo "firefox - signing key fingerprint verification failed!" >> ~/install-errors.log
